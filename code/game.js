    ...
    for (var y = 0; y < this.height; y++) {
        ...
        for (var x = 0; x < this.width; x++) {
            ...
            // Create a new actor at that grid position.
                if (ch == "F") {
                    this.finishActor = new Actor(new Vector(x, y), ch);
                } else {
...
function Player(pos) {
    ...
    this.initPos = pos.plus(new Vector(0, -0.5));
...
// Teleport is initialized based on the character, but otherwise has a
// size and position
function Teleport(pos, ch) {
    this.pos = pos;
    this.size = new Vector(1, 1);
    if (ch == "g") {
        // Horizontal teleport
        ...
        // Drip teleport. Repeat back to this pos.       
        this.speed = new Vector(0, 3);
        this.repeatPos = pos;
    }
}
...
Level.prototype.obstacleAt = function(pos, size) {
    ...
    for (var y = yStart; y < yEnd; y++) {
        for (var x = xStart; x < xEnd; x++) {
            ...
            if (fieldType && fieldType != "floater2" && fieldType != "floater3" && fieldType != "coin2" && fieldType != "wallfake") return fieldType;
        }
    }
};
...
Level.prototype.playerTouched = function(type, actor) {
    ...
    // Player loses
    if ((type == "lava" || type == "lavadrop" || type == "lavawalk") && this.status == null) {
        this.status = "lost";
        this.finishDelay = 1;
    } else if (type == "coin") {
        ...
        // If there aren't any coins left, create Win portal
        if (!this.actors.some(function(actor) {
            ...
            this.finishDelay = 1;
            */
            this.actors.push(this.finishActor);
        }
    } else if (type == "teleport" && this.status == null) {
        this.player.pos = this.player.initPos;
